...
namespace fv
{
    defineTypeNameAndDebug(actuatorLineElement, 1);
...
)
{
    ...
    {   
        /********add for not found**************/
        point minPoint = meshBoundBox_.min();
        point maxPoint = meshBoundBox_.max();
        point newMinPoint = minPoint - point(50, 30, 30);
        point newMaxPoint = maxPoint + point(50, 30, 30);
        boundBox expandedBox_(newMinPoint, newMaxPoint);
        /********add for not found**************/
        ...
        if (meshBoundBox_.containsInside(location))
        {
            if (debug)
            {
                Pout<< "[updated] Looking for cell containing " << location
        ...
        {
            /********add for not found**************/
            if (expandedBox_.containsInside(location))
            {
                if (debug)
                {
                    Pout<< "Looking for cell containing " << location
                        << " inside expanded bounding box:" << endl
                        << expandedBox_ << endl;
                }
                return mesh_.findCell(location);  
            }        
...
Foam::scalar Foam::fv::actuatorLineElement::calcProjectionEpsilon()
{
    ...
    scalar meshFactor = GaussianCoeffs.lookupOrDefault("meshFactor", 0.5);
...
)
{
    ...
    inflowVelocity_ = vector(VGREAT, VGREAT, VGREAT); //1E+37
