...
namespace fv
{
    defineTypeNameAndDebug(axialFlowTurbineALSource, 1);
...
void Foam::fv::axialFlowTurbineALSource::createBlades()
{
    ...
    for (int i = 0; i < nBlades_; i++)
    {
        ...
        // Add prescribed motion data
        bladeSubDict.add("prescribedMotion", prescribedMotionDict_);

        // Add rigid body floating motion data
        bladeSubDict.add("rigidBodyMotion", rigidBodyMotionDict_);
...
void Foam::fv::axialFlowTurbineALSource::createHub()
{
    ...
    hubSubDict.add("prescribedMotion", prescribedMotionDict_);

    // Add rigid body floating motion data
    hubSubDict.add("rigidBodyMotion", rigidBodyMotionDict_);
...
void Foam::fv::axialFlowTurbineALSource::createTower()
{
    ...
    // Add harmonic floating motion data
    towerSubDict.add("prescribedMotion", prescribedMotionDict_);

    // Add rigid body floating motion data
    towerSubDict.add("rigidBodyMotion", rigidBodyMotionDict_);
...
void Foam::fv::axialFlowTurbineALSource::calcEndEffects()
{
    ...
    forAll(blades_, i)
    {
        forAll(blades_[i].elements(), j)
        {
            ...
            vector elementVel = blades_[i].elements()[j].velocity() + blades_[i].elements()[j].motionVelocity();
            if (debug)
            {
    ...
    )
{
    ...
    // Initialise turbine orientation according to prescribed/rigidBody motion if specified
    motionInitialise();
...
void Foam::fv::axialFlowTurbineALSource::motionUpdate()
{
    // If prescribed or rigidBody motion is active, update turbine axis
    // and origin accordingly. The prescribed motion is only executed once per 
    // timestep, while the rigid body does every PIMPLE iteration
    if ((prescribedMotionActive_ && time_.value() != lastMotionTime_) || rigidBodyMotionActive_)
    {
        ...
        // current prescribed/rigidBody motion translation
        origin_ += blades_[0].motionTranslation();
        // Update current motion time
        lastMotionTime_ = time_.value();
    }
}


void Foam::fv::axialFlowTurbineALSource::motionInitialise()
{
    // If If prescribed/rigidBody motion is active, initialise accordingly
    if (prescribedMotionActive_ || rigidBodyMotionActive_)
    {
        // Access orientation values from actuator line
        // All blades from the same turbine have same
        // orientation, just acces first blade
        ...
        // Align with initial orientation, 
...
void Foam::fv::axialFlowTurbineALSource::computeRigidBodyLoads(scalar rhoRef, vector moment)
{
    // Forces to map into the rigid body
    vector rbForce = force_;

    // Account for tower force if includeTowerDrag_ = false
    if (hasTower_ && !includeTowerDrag_)
    {
        rbForce += tower_->force();
    }

    // Same for the Nacelle
    if (hasNacelle_ && !includeNacelleDrag_)
    {
        rbForce += nacelle_->force();
    }
    ...
    if (hasTower_)
    {
        rbMoment += tower_->moment(origin_);
    }
    if (hasNacelle_)
    {
        rbMoment += nacelle_->moment(origin_);
    }

    // Multiply loads by rho if flow is incompressible
    rbForce*= rhoRef;
    rbMoment*= rhoRef;
    
    writeRigidBodyLoads(rbForce, rbMoment, origin_);
}
...
)
{
    ...
    // Transfer loads onto the rigid body library
    if (coupleRigidBodyLoads_)
    {
        scalar rhoRef;
        coeffs_.lookup("rhoRef") >> rhoRef;
        computeRigidBodyLoads(rhoRef, moment);
    }
    ...
    thrustCoefficient_ = -force_ & axis_
                     / (0.5*frontalArea_*magSqr(freeStreamVelocity_));
...
)
{
    ...
    if (coupleRigidBodyLoads_)
    {
        // Compressible case, no need to multiply loads with density.
        // Thus, set reference density to 1
        computeRigidBodyLoads(1, moment);
    }
