...
:
    ...
    uWind_( vector(0., 0., 0.) ),
    z_wind0_( scalar(0) ),
    z_current0_( scalar(0) ),
    hub_height_( scalar(0.) ),
    random_fluctuation_( scalar(0.) ),
    genAbs_(false),
    secondOrder_(false),
    extrapolation_(true),
    nPaddles_(1),
...
:
    ...
    uWind_(ptf.uWind_),
    z_wind0_(ptf.z_wind0_),
    z_current0_(ptf.z_current0_),
    hub_height_(ptf.hub_height_),
    random_fluctuation_(ptf.random_fluctuation_),
    genAbs_(ptf.genAbs_),
...
:
    ...
    z_wind0_(dict.lookupOrDefault("zWind_0", scalar(0.01))),
    z_current0_(dict.lookupOrDefault("zCurrent_0", scalar(1.1))),
    hub_height_(dict.lookupOrDefault("hub_height", scalar(150))),
    random_fluctuation_(dict.lookupOrDefault("random_fluctuation", scalar(0.))),
...
void Foam::waveVelocityFvPatchVectorField::updateCoeffs()
{
    ...
    if (!allCheck_ || reread)
    {
        ...
        z_wind0_ = 
            (waveDict.lookupOrDefault<scalar>("zWind_0", scalar(0.01) ));
        z_current0_ = 
            (waveDict.lookupOrDefault<scalar>("zCurrent_0", scalar(1.1) ));
        hub_height_ = 
            (waveDict.lookupOrDefault<scalar>("hub_height", scalar(150) ));
        random_fluctuation_ = 
            (waveDict.lookupOrDefault<scalar>("random_fluctuation", scalar(0) ));
...
//    inlinePrint( "Measured Level ", measuredLevels );
    ...
    /*************************************Wind******************************************/
    Info << "Random logarithmic gradient wind generation:" << endl;
    
    const scalarField patchCf = patch().Cf().component(2);    
    
    const scalar z_wind_ref = hub_height_; //150.0; //Hub height
    const scalar z_wind_0 = z_wind0_;//0.01; // Surface roughness length	
    const scalar z_wind_ground = 0.0; //Water level  
    vectorField U_wind = vector(0., 0., 0.) * patchCf;
    const scalar Ran_fluc = random_fluctuation_; //0.02;
    
    Info << "Hub height:" << z_wind_ref  <<  endl;
    Info << "Wind random fluctuation:" << Ran_fluc*Ran_fluc  <<  endl;
    ...
    forAll(U_wind, i)
    {
        ...
        if (z > z_wind_0) // Above ground height
        {			
	    std::random_device rd;
	    std::mt19937 gen(rd());
            std::uniform_real_distribution<> dis(1-Ran_fluc,1+Ran_fluc);
	    ...
	    //Log profile	
            U_wind[i] = uWind_ * log((z-z_wind_ground)/z_wind_0)/log((z_wind_ref-z_wind_ground)/z_wind_0) * randomNumber1 * randomNumber2;
				
        } 
        else U_wind[i] = vector(0., 0., 0.);	
    };
    ...
    operator==
        ...
        + n3*patchW*timeMult 
        + U_current * neg(0.9-alphaCell)    
        + U_wind * pos(0.9-alphaCell)    
